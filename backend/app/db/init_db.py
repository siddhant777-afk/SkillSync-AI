...
def init_db(db: Session) -> None:
    ...
    # Also seed a demo recruiter account for testing candidate search
    recruiter = db.query(User).filter(User.email == "recruiter@techhire.com").first()
    if not recruiter:
        recruiter = User(
            email="recruiter@techhire.com",
            hashed_password=get_password_hash("password123"),
            full_name="Alex Morgan (Talent Lead)",
            role="recruiter",
            is_verified=True,
        )
        db.add(recruiter)

    # Seed an additional student to demonstrate recruiter filtering
    second_student = db.query(User).filter(User.email == "rahul.verma@example.com").first()
    if not second_student:
        ...
        p2 = StudentProfile(
            ...
            bio="Full Stack Developer with expertise in React, Node.js and distributed databases.",
            career_goal="Backend Developer",
            placement_readiness=89,
            profile_completion=95,
        )
        db.add(p2)
        p2_accounts = ConnectedAccounts(
            user_id=second_student.id,
            github_username="rahulv",
            leetcode_username="rahul_code",
            codeforces_username="rahul_cf",
        )
        db.add(p2_accounts)
        db.add(
            PlatformStats(
                user_id=second_student.id,
                platform="leetcode",
                stats_data={"username": "rahul_code", "solved": 650, "rank": "Top 8%"},
            )
        )
        db.add(
            PlatformStats(
                user_id=second_student.id,
                platform="github",
                stats_data={"username": "rahulv", "contributions": 940, "repositories": 25},
            )
        )
        db.add(Skill(user_id=second_student.id, name="Go / Golang", level=85, category="Backend", status="Strong"))
        db.add(Skill(user_id=second_student.id, name="PostgreSQL", level=90, category="Databases", status="Strong"))
        db.add(Skill(user_id=second_student.id, name="Docker", level=80, category="DevOps", status="Strong"))
